...
# The zone apex, the www record and every Cloudflare ruleset (config, cache,
# redirect, response headers) belong to the static site stack in site/, which
# has its own state. Rulesets are one entrypoint per phase per zone: a rule for
# api/bff in one of those phases must be added there, not here.
