...
environment variables. **A bug on the apex site cannot reach finance data.**
...
- A Cloudflare zone for your domain and an API token with **DNS edit** on it —
  plus **Zone Settings edit** if `manage_cloudflare_zone_security = true`.
...
- **`cloudflare_ssl_mode = "strict"`** requires a valid, publicly trusted
  ...
  safe here — but any *other* origin already in the zone must too, or it starts
  returning 526.
...
- **CAA records restrict who may issue certificates for the whole zone.** The
  ...
  months later. `hsts_preload` is likewise slow to undo — leave it `false` until
...
| `web_domain_name`                 | `""`              | Apex, e.g. `finances.app.br`.                |
...
destroy outright. The Cloudflare zone settings survive — see above.
